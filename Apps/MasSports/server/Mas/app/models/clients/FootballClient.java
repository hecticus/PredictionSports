...
public class FootballClient extends Client {
    ...
    public FootballClient(Client client){
        super(client.getIdClient(), client.getNickname(), client.getFacebookId(), client.getSession(), client.getLastUpdate(), client.getDevices()  , client.getUserId(), client.getStatus(), client.getLogin(), client.getPassword(), client.getCountry(), client.getLastCheckDate(), client.getLanguage());
        
        this.pushAlerts = ClientHasPushAlerts.finder.where().eq("id_client", getIdClient()).findList();
        this.leaderboards = Leaderboard.finder.where().eq("id_client", getIdClient()).findList();
        this.leaderboardGlobal = LeaderboardGlobal.finder.where().eq("id_client", getIdClient()).findList();
        this.leaderboardTotal = LeaderboardTotal.finder.where().eq("id_client", getIdClient()).findUnique();
        this.leaderboardPushs = LeaderboardPush.finder.where().eq("id_client", getIdClient()).findList();
        this.clientBets = ClientBets.finder.where().eq("id_client", getIdClient()).findList();
    ...
    public List<ClientBets> getClientsBet()
    {
        if(this.clientBets.size() == 0 )
            clientBets = ClientBets.finder.where().eq("id_client", getIdClient()).findList();
        return this.clientBets;
    }
