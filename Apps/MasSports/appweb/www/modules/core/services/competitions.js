...
angular
    ...
    .factory('Competitions',['$localStorage', '$http', '$q', 'Domain', 'WebManager', 'Client',
        function($localStorage, $http, $q, Domain, WebManager, Client) {
            ...
            function getCompetitionsBaseball(noConfig) {
                loadCompetitions();
                var config = WebManager.getFavoritesConfig(Client.isFavoritesFilterActive());

                if (noConfig) config = false;

                return $http.get(Domain.competitionsbaseball(), config).then(function(response){
                    competitions = response.data.response.competitions;
                    saveCompetitions();
                    return competitions;
                },function(response){
...
//                        console.log(response);
            ...
            function getRankingBase(competition){
            console.log("getRanking-> " + Domain.rankingbase(competition));
                return $http.get(Domain.rankingbase(competition))
                    .then(function (response) {
//                        console.log('getRanking.response: ');
//                        console.log(response);
                        if(response.data.error === 0){
            ...
            return {
                 ...
                 */
                get: getCompetitions,
                getbaseball: getCompetitionsBaseball,
                getPrediction: getCompetitionsPrediction,
                getRanking : getRanking,
                getRankingBase : getRankingBase,
                            ...
                            return $http.get(Domain.leaderboard.personal.competition())
                                .then(function(response){
                                    ...
                                    $http.get(Domain.competitions()).then(function(response){
                                        ...
                                        //Cambios de fernando para ahcer magia
                                    $http.get(Domain.competitionsbaseball()).then(function(response){

                                        var allCompetitions = response.data.response.competitions;

                                        leaderboard.map(function(score){
                                            allCompetitions.some(function(competition){
                                                if((competition.id_competitions === score.id_tournament)  && (competition.id_competitions === 1 || competition.id_competitions === 24)){
                                                    score.name = competition.competiton_type.name;
                                                    score.logo = competition.competiton_type.competition_logo;
                                                    return true;
                                                }
                                    ...
                                    //final de cambios de fernando para hacer magia
