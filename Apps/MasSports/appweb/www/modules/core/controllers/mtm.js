...
angular
    ...
    .controller('MtmCtrl', ['$http','$rootScope','$scope','$state','$localStorage', '$interval',  '$stateParams', 'WebManager',
        ...
        function($http, $rootScope, $scope, $state, $localStorage, $interval, $stateParams, WebManager,
            ...
            $scope.refreshEventsBase = function (competitionId, matchId) {
                ...
                $rootScope.refreshInterval = null;
                  ...
                  $http.get(Domain.mtmbase(competitionId, matchId, _event.first), config)
                      .then(refreshSuccess, refreshError);
            ...
            $scope.showContentEventsBase = function (_league, _match) {

                //if ((_match.id_status === 1) ||  (_match.id_status === 2)) {

                  $scope.itembase.mtm = [];
                  $scope.itembase.league = _league;

                  $scope.itembase.match = {
                      home: {name:_match.home_team.name, goals:_match.home_team_goals, logo:_match.home_team.team_logo},
                      away: {name:_match.away_team.name, goals:_match.away_team_goals, logo:_match.away_team.team_logo},
                      status: {id_status:_match.id_status,name:_match.status}
                  };
                  ...
                  $scope.matchId = matchId;
                  _event.reset();
                  $scope.refreshEventsBase(competitionId, matchId);
                //}
            };
            ...
            function mapLeaguesBase(leagues){
                leagues.forEach(function(league){
                    league.fixtures.map(function(match){
...
/*
                        $http.get(Domain.matchbaseball(date), config).then(
                            function (data) {
                                $scope.hasGamesForToday = true;
                                var response = data.data.response;
                                if(response && response.leagues.length > 0){
                                    $scope.hasGamesForToday = true;
                                    mapLeaguesBase(response.leagues);
                                    $scope.itembase = response;

                                    if($stateParams.matchId){
                                      $scope.itembase.leagues.forEach(function(league){
                                        league.fixtures.forEach(function(match){
                                              if ($stateParams.matchId ==match.id_game_matches) {
                                                $scope.showContentEvents(league, match);
                                              }
