...
angular
    ...
    .controller('SettingsController', [
        ...
        function($scope, $rootScope, $localStorage, $state, $timeout, $translate, $stateParams, ClientManager, TeamsManager
            , FacebookManager, Settings, iScroll, i18n, Client, Notification, CordovaDevice, flash) {
            ...
            {
                var r = confirm("¿Está seguro que quiere darse de baja?");
                    $scope.$emit('load');

                if(r){

                    ClientManager.getDown($scope.GetDownEnd);

                    //alert("start to get Down");
                }
            }


            $scope.GetDownEnd = function (r)
            {
                $scope.$emit('unload');
                if(r){
                    $rootScope.executeAction('logout');
                }
                ...
                //if(!!window.facebookConnectPlugin) {
                    ...
                    FacebookManager.getStatus(function (result) {
                        if (result) {
                            ...
                            $scope.setFbButtonMsg();
                            console.log("audel fb log: status is " + result.status);
                        }
                    });
                /*} else {
                    $scope.setFbButtonMsg();
