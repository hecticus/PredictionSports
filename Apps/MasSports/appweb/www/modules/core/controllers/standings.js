...
angular
    ...
    var rankingbaseScroll = null;
            var rankingbaseScroll2 = null;
            ...
            $scope.showbaseballleague = false;
            ...
            $scope.showContentPhasesBase = function(competition) {
                $scope.$emit('load');
                $scope.item.ranking = [];
                $scope.item.phases = [];
                $scope.item.competition = false;
                ...
                //$scope.item.phases = phases;
                $scope.item.competition = competition;
                $scope.showContentRankingBase($scope.item.competition.id_competitions);
            ...
            $scope.showContentleagye = function() {
                //debugger;
                //$rootScope.transitionPageBack('#wrapper5', 'left');
                //rankingbaseScroll.scrollTo(0,0,0);
                $rootScope.transitionPageBack('#wrapper2', 'left');
                phaseScroll.scrollTo(0,0,0);
                $scope.showbaseballleague = true;
                $scope.$emit('unload');
            ...
            $scope.showContentRankingBase = function(competition) {
                $scope.$emit('load');
                $scope.item.phase = false;
                $scope.item.ranking = [];

                Competitions.getRankingBase(competition)
                    .then(function (response) {
                        $scope.hasRankings = true;
                        $scope.item.isTree = response.tree;
                        $scope.item.phase = response.phase;
...
$scope.isbaseball = true;
            ...
            function setUpIScroll(){
                //rankingbaseScroll = iScroll.vertical('wrapper5');
                tournamentScroll = iScroll.vertical('wrapper');
                phaseScroll = iScroll.vertical('wrapper2');
                rankingScroll = iScroll.vertical('wrapper3');
                //rankingScroll = iScroll.vertical('wrappertmp');
                

                $scope.$on('$destroy', function() {

                    //rankingbaseScroll.destroy();
                    //rankingbaseScroll = null;
            ...
            function getCompetitions(){
                Competitions.get().then(function(competitions){
                    ...
                    Competitions.getbaseball().then(function(competitions){
                        $scope.hasCompetitions = true;
                        $scope.itembase.competitions = competitions;
                        $scope.$emit('unload');
                    }, function(){
                        //$scope.hasCompetitions = false;
                        $scope.itembase.competitions = [];
                        //Notification.showNetworkErrorAlert();
                        $scope.$emit('unload');
                    });
